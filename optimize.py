...
def semantic_dpw_objective(trial, path, limit):
    ...
    jt = trial.suggest_float('jt', 0.0, 1.0, step=0.01)
    st = trial.suggest_float('st', 0.0, 1.0, step=0.01)
    ...
    return min(mm.mean_average_precision(relevant, received_cosine),
    mm.mean_average_precision(relevant, received_jaccard))
